...
export function PlansPanel({ plans, companies, writeCompanyId, onChanged }: { plans: Plan[]; companies: Company[]; writeCompanyId: number; onChanged: () => void }) {
  ...
  return <section className="plans-module plans-catalog stack">
    ...
    <Modal title={editingPlan ? 'Editar plan comercial' : 'Crear plan comercial'} open={modalOpen} onClose={closeModal}><form className="plan-modal-form" onSubmit={savePlan}><div className="plan-modal-intro"><span>{editingPlan ? 'Editar' : 'Nuevo'}</span><p>Define la oferta comercial y sus condiciones principales.</p></div><div className="plan-form-grid">
